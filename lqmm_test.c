...
double pdMat(double *z, double *V, double *psi, int *q, int *N, int *Kq, int k, int i){
...
*/
	...
	double resid, val, mvalk, LOGC, TMP_VAR;
	double _cons;
...
// Asym. Laplace Likelihood	for M clusters
	...
	{
		if(weights[i] > 0){
		_cons = log((*quantile) * (1-(*quantile)) / sigma[i]);
		
		ls = start[i];
		le = end[i];
		TMP_VAR = 0.0;
		TMP_VAR = pdMat(z, V, Tq, q, N, Kq, k, i);
				...
				for (j = ls; j < le; j++)
				{
					...
					resid = w[j] - TMP_VAR;
					val +=_cons- (1 / (2 * sigma[i])) * (fabs(resid) + (2 * (*quantile) - 1) * resid);
				}
				}
	}
...
void lamC_ll_h(double theta[], double *x, double *y, double *z, double *weights, double *lambda,double *Tq, double *V, double *W, double *sigma, float *quantile, int *p, int *q, int *m, int *M, int *N, int *Kq, int *start, int *end, double *loglik)
{
...
void invlamC_ll_h(double theta[], double *x, double *y, double *z, double *weights, double *lambda,double *Tq, double *V, double *W, double *sigma, float *quantile, int *p, int *q, int *m, int *M, int *N, int *Kq, int *start, int *end, double *loglik)
{
...
// Asym. Laplace Likelihood	for M clusters
	...
	for (i = 0; i < *M; i++)
	{
		...
		val+=-*lambda * (1/sigma[i]) * (1/sigma[i]) ;
				}
	}
	valk[k] = val + log(fabs(W[k]));
			if(valk[k] > LOGC)	LOGC = valk[k]; // constant for OVERFLOW protection
...
void sig(double *theta, double *x, double *y, double *z, double *weights, double *Tq,  double *V,double *W, float *quantile,  int *p, int *q, int *m, int *M, int *N, int *Kq, int *start, int *end, double *sigma){
	int i, k, j, ls, le, loc_N, loc_Kq;
	double resid, val, TMP_VAR;
		...
		val = 0.0;
			...
			//valk[k] = 0;
				for (j = ls; j < le; j++)
				{
					...
					resid = w[j] - TMP_VAR;
					val +=  (weights[i] / (2* (le-ls)* pow((double) loc_Kq,(1/(double) *q)))) * (fabs(resid) + (2 * (*quantile) - 1) * resid);
				}
			}
		sigma[i]=val ;
		}
